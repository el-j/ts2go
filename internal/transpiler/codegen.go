...
func (g *CodeGenerator) generateConstructorStatement(node *ASTNode, instanceVar string) error {
	...
	if node.Kind == ExpressionStatement {
		...
		if len(node.Children) > 0 && node.Children[0].Kind == BinaryExpression {
			...
			if len(binary.Children) >= 2 {
				...
				if left.Kind == PropertyAccessExpression && len(left.Children) > 0 {
					if left.Children[0].Kind == ThisKeyword {
...
func (g *CodeGenerator) generatePropertyAccess(node *ASTNode) (string, error) {
	...
	if node.Children[0].Kind == ThisKeyword && g.currentClassMembers != nil {
	...
	if node.Children[0].Kind == Identifier {
		objectName := node.Children[0].Text
...
func (g *CodeGenerator) generateNewExpression(node *ASTNode) (string, error) {
	...
	if len(node.Children) > 1 {
		for _, arg := range node.Children[1:] {
			// Skip the class identifier itself
			if arg.Kind == Identifier && toPascalCase(arg.Text) == className {
				continue
			}
			argStr, err := g.generateExpression(&arg)
			if err != nil {
