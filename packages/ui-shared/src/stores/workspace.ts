...
export const useWorkspaceStore = defineStore('workspace', () => {
      ...
      if (parts.length === 1) {
        ...
        for (let i = 0; i < parts.length - 1; i++) {
          ...
          let folder = currentLevel.find(
            n => n.path === currentAbsolutePath && n.type === 'folder'
          );
          if (!folder) {
            folder = {
              name: part,
  ...
  function setTranspilation(
    ...
    const status: TranspilationStatus = errorMessage
      ? 'error'
      : warningMessage
        ? 'warning'
        : success
  ...
  function updateTranspilationStatus(
    tsFilePath: string,
    status: TranspilationStatus,
    errorMessage?: string,
    warningMessage?: string
  ...
  function finishTranspilation() {
    ...
    addTranspilationLog(
      `Transpilation complete: ${completedFiles}/${totalFiles} files processed`,
      'info'
    );
  }
  ...
  function updateFileTranspiledStatus(
    filePath: string,
