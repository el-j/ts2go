...
export function useBackendState<T>(key: string, defaultValue: T) {
  ...
  const load = async (): Promise<T> => {
    try {
      if (isWeb) {
        const jsonString = localStorage.getItem(`ts2go:${key}`)
  ...
  const save = async (data: T): Promise<void> => {
    try {
      ...
      if (isWeb) {
        localStorage.setItem(`ts2go:${key}`, jsonString)
      } else {
...
export function useBackendSettings<T>(defaultValue: T) {
  ...
  const load = async (): Promise<T> => {
    try {
      if (isWeb) {
        const jsonString = localStorage.getItem('ts2go:settings')
        if (!jsonString || jsonString === '{}') return defaultValue
        return JSON.parse(jsonString) as T
      } else {
  ...
  const save = async (data: T): Promise<void> => {
    try {
      const jsonString = JSON.stringify(data)
      if (isWeb) {
        localStorage.setItem('ts2go:settings', jsonString)
      } else {
