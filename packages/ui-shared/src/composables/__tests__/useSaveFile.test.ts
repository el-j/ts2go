...
describe('useSaveFile', () => {
  beforeEach(() => {
    ...
    // Force Tauri mode so invoke() is used rather than fetch() web path
  ...
  it('should create backups when enabled in settings', async () => {
    ...
    // Must have called write_file for the actual file
    expect(invoke).toHaveBeenCalledWith('write_file', {
      path: '/test/file.ts',
      content: 'const x = 1;',
    });

    // Backup call: write_file with path containing '.backups'
    const backupCall = vi
      .mocked(invoke)
      .mock.calls.find(
        c => c[0] === 'write_file' && (c[1] as Record<string, string>).path.includes('.backups')
      );
  ...
  it('should save all dirty files', async () => {
    ...
    // write_file must have been called for each file (may have extra calls from TS validation)
    expect(invoke).toHaveBeenCalledWith('write_file', {
      path: '/test/file1.ts',
      content: 'const x = 1;',
    });
